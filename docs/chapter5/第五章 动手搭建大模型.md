...
section: 第二部分 / 实战
order: 500
tags: [llama, pytorch, tokenizer, pretraining]
aliases: [动手搭建大模型]
prerequisites: [large-language-models]
summary: 用 PyTorch 逐步实现 LLaMA 风格模型、Tokenizer、预训练与监督微调流程。
status: stable
---
...
preset: project-smoke
```

> [!experiment]
> 推荐先运行默认“离线工程检查”，确认七个脚本与演示数据完整；它不会联网或训练。随后可切换到“CPU 训练小型分词器”。预训练和 SFT 预设需要自行准备 JSONL 数据与 CUDA 环境，并在启动前检查参数；实验跟踪默认关闭，产物统一保存在 `code/.tensornote-runs/`。

> [!bridge]
> 学习路径：[[large-language-models|第四章 大语言模型]] → 本章 → [[llm-training-practice|第六章 大模型训练流程实践]]。本章完整训练需要额外依赖与算力，普通代码 Fence 不会自动执行。
