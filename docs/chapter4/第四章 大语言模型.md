...
section: 第一部分 / 理论基础
order: 400
tags: [llm, scaling, capability, alignment]
aliases: [大语言模型]
prerequisites: [pretrained-language-models]
summary: 理解大语言模型的定义、训练阶段、能力表现与使用边界。
status: stable
---
...
> [!bridge]
> 学习路径：[[pretrained-language-models|第三章 预训练语言模型]] → 本章 → [[sampling-lab|温度与 Top-k 采样实验]] → [[build-an-llm|第五章 动手搭建大模型]]。
