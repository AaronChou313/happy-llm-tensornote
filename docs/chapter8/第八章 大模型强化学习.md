---
id: llm-reinforcement-learning
title: 第八章 大模型强化学习
section: 第二部分 / 实战
order: 800
tags: [reinforcement-learning, grpo, agentic-rl]
aliases: [大模型强化学习, Agentic RL]
prerequisites: [preference-alignment, llm-applications]
summary: 理解 GRPO、OPD、Search-R1 与 ReTool 如何把可验证反馈用于语言模型和 Agent 训练。
status: growing
---
...
> [!bridge]
> 学习路径：[[llm-applications|第七章 大模型应用]] → 本章 → [[grpo-lab|组内相对优势实验]] → [[happy-llm-course-home|课程主页]]复盘。完整训练代码是重型实验，不会在 TensorNote 中自动运行。

```tensornote-experiment
manifest: ./tensornote.experiment.yaml
preset: workflow-smoke
```

> [!experiment]
> 首次阅读先运行默认 smoke：它只检查四组脚本并用固定奖励计算 GRPO 组内优势，不联网、不训练。其余预设均需先完成 `trio login`，会访问数据集或 PyTRIO 远端服务并可能产生费用。TensorNote 默认关闭 SwanLab。ReTool 会执行模型生成的 Python，只能在无凭证、低权限的一次性容器或虚拟机中启动。
...
配套代码位于 `docs/chapter8`。在 TensorNote 中可直接切换实验预设；若脱离 TensorNote 手动运行，则从仓库根目录安装公共依赖：
