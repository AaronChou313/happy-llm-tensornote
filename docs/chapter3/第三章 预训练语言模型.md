...
section: 第一部分 / 理论基础
order: 300
tags: [pretraining, encoder, decoder, language-model]
aliases: [预训练语言模型]
prerequisites: [transformer-architecture]
summary: 比较 Encoder-only、Encoder-Decoder 与 Decoder-only 模型的训练目标和适用任务。
status: stable
---
...
> [!bridge]
> 学习路径：[[transformer-architecture|第二章 Transformer 架构]] → 本章 → [[large-language-models|第四章 大语言模型]]。
