...
section: 第二部分 / 实战
order: 700
tags: [evaluation, rag, agent, application]
aliases: [第七章 大模型应用]
prerequisites: [llm-training-practice]
summary: 将大模型用于评测、检索增强生成与工具型 Agent，并理解各模块的数据流。
status: stable
---
...
> [!bridge]
> 学习路径：[[llm-training-practice|第六章 大模型训练流程实践]] → 本章 → [[llm-reinforcement-learning|第八章 大模型强化学习]]。RAG 与 Agent 示例可能需要外部模型、服务或密钥，请只在隔离环境中手动配置。
...
> [!experiment]
> 建议先运行默认的“离线 RAG 原理实验”，它不会联网或读取密钥。随后可切换到“离线 Agent 工具实验”。两个 smoke 都通过后，再在 TensorNote 的受信任环境中配置 `OPENAI_API_KEY` 与 `OPENAI_BASE_URL`，显式启动 API 预设。API 调用可能产生费用；仓库不会保存密钥。RAG 完整预设使用内置短文档，产物会写入 `.tensornote-runs/`。
